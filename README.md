...
面向 Mihomo、sing-box 与主流代理客户端的订阅生成器和特化分流规则。同一套代码有两种形态：在自己的电脑上用 Docker 生成完整配置，真实订阅不出本机；或者部署成一个**什么都不保存**的开放转换站，每个访问者在浏览器里拼出属于自己的订阅链接。公开规则也可以直接用于第三方 Subconverter 或 Mihomo 原生模板。
...
| **开放转换站** | 不方便运行 Docker，但不想把订阅交给会保存它的服务 | 站点服务器（**不保存**，转换完即丢） | 自带全部参数的订阅链接 |
| 第三方在线转换 | 不方便运行 Docker、接受信任转换后端 | 第三方后端 | 第三方订阅 URL |
| Mihomo 原生模板 | 只需要 Ekko Rules，其他客户端设置自己维护 | Mihomo 客户端 | Provider 模板 |

两种自托管形态的区别只有一个开关 `SELFHOST_MODE`：`lan` 保存固定档案、真实订阅不进 URL；`public` 不保存任何东西，代价是订阅写在链接里。详见 [selfhost/README.md](selfhost/README.md) 与 [部署开放转换站](selfhost/docs/vps.md)。
