...
完了。需要的话在「高级选项」里勾 UDP、XUDP 这类开关，链接会当场跟着变；也可以复制链接或扫码。Shadowrocket 例外：页面会明确给出 **① 节点订阅、② 分流配置** 两个按钮和两张二维码，必须按顺序都导入。第 1 步在首页建立可刷新的节点订阅、名称和流量/到期横幅，并先选好一个可用节点；第 2 步在「配置」页导入规则、策略组与 `DIRECT` / `REJECT`，其中原生 `PROXY` 会调用首页当前节点。
