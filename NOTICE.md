...
| Current specialized, private/local, and service corpus | 2,409 | canonical rules plus migration/review history |
| **Total file rules** | **7,461** | `sources/quality-baseline.yaml` |

The 2,409-rule current specialized corpus is not claimed to be entirely newly authored. It combines reconstructed factual indicators with substantial local rebuilding, additions, deletions, retargeting, and precision corrections; the initial expanded profile did not preserve per-rule source attribution.
