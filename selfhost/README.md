...
**一个二维码，按入口提供对应内容。** 多数客户端的扫码入口把 `http(s)` 地址写进自身配置，而手机系统相机会在浏览器中打开同一地址，再由中转页唤起客户端。Shadowrocket 二维码继续使用已验证能从首页和「配置」页一次导入节点与配置的完整 HTTPS `.yaml` 地址；规则和策略组保留在主配置中，节点改由主配置引用的同名远程 provider 提供。provider 地址返回用户填写的名称以及流量、到期响应头，从而避免纯 `shadowrocket://add/sub://…` 深链丢失配置，也不再依赖主配置地址的域名作为节点订阅名。弹窗另列原生 `.conf` 地址，供配置页手动导入。
...
Shadowrocket 的「配置」页使用原生分段 `.conf`。转换时先按所选远程配置生成完整的 `[Proxy]`、`[Proxy Group]`、`[Rule]` 骨架，再从无损的 Mihomo 节点结果补回旧版 Surge 输出会过滤的 AnyTLS、TUIC、VLESS Reality 等节点；每个 `select` 组还会写入 `policy-select-name`，Ekko Rules 的 `♻️ 手动切换`不写 `hidden`，确保它在代理分组列表中可见。扫码二维码使用普通 HTTPS `.yaml`：规则与策略组位于主配置，节点位于主配置引用的同名远程 provider，并沿用同一组策略成员关系。两种格式使用相同的源和模板。现代协议已做结构保留测试，尚未逐个完成真机连通验证。内置 Ekko Rules 完整版、精简版、第三方预设和允许的自定义远程配置都会走同一套转换；第三方模板保留其自身分组、规则和默认顺序。
...
订阅响应会通过 `Profile-Title` 和 `Content-Disposition` 同时传递用户填写的名称；Shadowrocket 原生配置和完整订阅地址分别把名称写进 `.conf`、`.yaml` 路径段，完整订阅地址也附上 `remark`。针对 0.4.10 暴露的首页首次扫码显示域名、配置页首次扫码缺少横幅，完整 YAML 还会创建一个以用户名称命名的远程 provider；节点、名称和订阅用量由 provider 的独立响应统一提供，主配置只负责规则与策略组。响应头中的名称不带扩展名；页面上的“下载”按钮走 `/api/convert`，那才是真的文件，仍然带扩展名。创建成功后页面不会清空真实订阅、名称或高级选项，用户可以直接微调后再次生成；完全相同的配置再次复制、扫码或一键导入时只复用现有浏览器记录，不会重复新增。

固定地址会把上游 `Subscription-Userinfo` 中 `upload`、`download`、`total` 和 `expire` 数字字段安全透传给客户端。若上游没有响应头，但节点订阅正文有格式完整的 `STATUS=↑/↓/TOT/Expires` 行，则会在移除该行供引擎读取之前转换为同样的响应头；单凭“剩余流量”节点名不能反推套餐总量。Shadowrocket 完整配置及其节点 provider 都返回相同的订阅用量，首页实际横幅由节点 provider 维护；原生 `.conf` 手动导入不承诺首页横幅。刷新时会沿用目标格式安全的上游 User-Agent。
