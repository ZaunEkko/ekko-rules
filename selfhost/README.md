...
**一个二维码，按入口提供对应内容。** 多数客户端的扫码入口把 `http(s)` 地址写进自身配置，而手机系统相机会在浏览器中打开同一地址，再由中转页唤起客户端。Shadowrocket 二维码直接放完整 HTTPS `.yaml` 地址：真机曾验证首页和「配置」页任一入口扫码一次都会导入节点与配置。后来的 `shadowrocket://add/sub://…` 深链虽能带来具名节点和流量横幅，但真机验证它在「配置」页只导入节点，首页扫码无反应，故不能作为完整配置的主二维码。完整 HTTPS 地址附加 `remark=<名称>` 作为名称提示；客户端是否采用、是否在配置页显示横幅，仍须真机验收。弹窗另列原生 `.conf` 地址，供配置页手动导入。
...
订阅响应会通过 `Profile-Title` 和 `Content-Disposition` 同时传递用户填写的名称；Shadowrocket 原生配置和完整订阅地址分别把名称写进 `.conf`、`.yaml` 路径段，完整订阅地址也附上 `remark`。0.4.10 的真机结果是首次首页扫码横幅存在但节点订阅名称显示域名；首次配置页扫码名称正确但首页横幅缺失。名称提示是否能改变这两个结果仍须真机验收，不能把响应头存在当作客户端显示正确。响应头中的名称不带扩展名；页面上的“下载”按钮走 `/api/convert`，那才是真的文件，仍然带扩展名。创建成功后页面不会清空真实订阅、名称或高级选项，用户可以直接微调后再次生成；完全相同的配置再次复制、扫码或一键导入时只复用现有浏览器记录，不会重复新增。
