...
1. 粘贴真实订阅地址；
2. 选择目标客户端，按需展开高级选项；
3. 复制固定 URL，或用手机扫描本地生成的二维码后导入客户端。弹窗里可以在「客户端里扫」（二维码内容是原始订阅地址）与「系统相机扫」（内容是客户端自己的导入 scheme）之间切换，默认选中该客户端真正能导入的那一种。两条路径最终都创建同一个可更新的远程订阅。
...
**一个二维码，按入口提供对应内容。** 多数客户端的扫码入口把 `http(s)` 地址写进自身配置，而手机系统相机会在浏览器中打开同一地址，再由中转页唤起客户端。Shadowrocket 二维码继续使用已验证能从首页和「配置」页一次导入节点与配置的完整 HTTPS `.yaml` 地址；主配置同时保留首页订阅刷新所需的内联节点、完整规则与策略组，并声明配置页可识别的同名远程 provider。provider 地址返回用户填写的名称以及流量、到期响应头，从而避免纯 `shadowrocket://add/sub://…` 深链丢失配置。首页扫码创建的订阅别名仍由 Shadowrocket 自身决定，服务端响应只能提供节点和流量横幅。弹窗另列原生 `.conf` 地址，供配置页手动导入。
...
二维码使用的远程地址把转换参数打包为 base64url 参数（通常是 `/i?p=…`），避免嵌套 scheme 时丢失机场 token。Shadowrocket 原生配置使用 `/i/<名称>.conf?p=…`，完整订阅使用 `/i/<名称>.yaml?srhome=1&p=…&remark=<名称>`；两条地址使用相同的订阅、选项与远程配置，但响应格式不同。`remark` 是客户端名称提示，不参与服务端配置解析；内部 `p` 参数仍是 base64url。

**一键导入按钮**在手机上直接访问站点时最省事，覆盖各家自己公开的 scheme：Clash / Mihomo `clash://install-config?url=`、Shadowrocket 原生配置 `shadowrocket://config/add/<地址>`、sing-box `sing-box://import-remote-profile?url=…#名称`、Surge `surge:///install-config?url=`、Loon `loon://import?sub=`、Surfboard `surfboard:///install-config?url=`。Shadowrocket 一键导入安装原生配置，不承诺首页订阅横幅；需要两个位置同时导入时请使用上面的完整 HTTPS 二维码。Quantumult X 的 `update-configuration` 只接受远程资源而不是整份配置，Quantumult 与 Mellow 没有公开 scheme，这三个只给复制地址。
...
Shadowrocket 的「配置」页使用原生分段 `.conf`。转换时先按所选远程配置生成完整的 `[Proxy]`、`[Proxy Group]`、`[Rule]` 骨架，再从无损的 Mihomo 节点结果补回旧版 Surge 输出会过滤的 AnyTLS、TUIC、VLESS Reality 等节点；每个 `select` 组还会写入 `policy-select-name`，Ekko Rules 的 `♻️ 手动切换`不写 `hidden`，确保它在代理分组列表中可见。扫码二维码使用普通 HTTPS `.yaml`：主配置保留内联节点供首页订阅刷新，规则与策略组引用同名远程 provider，使配置页继续保留填写的名称与用量横幅。两种格式使用相同的源和模板。现代协议已做结构保留测试，尚未逐个完成真机连通验证。内置 Ekko Rules 完整版、精简版、第三方预设和允许的自定义远程配置都会走同一套转换；第三方模板保留其自身分组、规则和默认顺序。
...
订阅响应会通过 `Profile-Title` 和 `Content-Disposition` 同时传递用户填写的名称；Shadowrocket 原生配置和完整订阅地址分别把名称写进 `.conf`、`.yaml` 路径段，完整订阅地址也附上 `remark`。完整 YAML 还会创建一个以用户名称命名的远程 provider；provider 为配置页保留名称与订阅用量，主 YAML 的内联节点则保证首页订阅可以刷新。Shadowrocket 首页扫码创建的 Alias/Remark 不采用这些响应字段时仍会显示域名，这是客户端本地命名行为。响应头中的名称不带扩展名；页面上的“下载”按钮走 `/api/convert`，那才是真的文件，仍然带扩展名。创建成功后页面不会清空真实订阅、名称或高级选项，用户可以直接微调后再次生成；完全相同的配置再次复制、扫码或一键导入时只复用现有浏览器记录，不会重复新增。

固定地址会把上游 `Subscription-Userinfo` 中 `upload`、`download`、`total` 和 `expire` 数字字段安全透传给客户端。若上游没有响应头，但节点订阅正文有格式完整的 `STATUS=↑/↓/TOT/Expires` 行，则会在移除该行供引擎读取之前转换为同样的响应头；单凭“剩余流量”节点名不能反推套餐总量。Shadowrocket 完整配置及其节点 provider 都返回相同的订阅用量：首页扫码由主响应维护横幅，配置页导入由 provider 维护横幅；原生 `.conf` 手动导入不承诺首页横幅。刷新时会沿用目标格式安全的上游 User-Agent。
...
- 订阅把节点放在 `proxy-providers` 里时，网关会自己去拉那些 provider（最多 8 个），走与订阅同一套私网地址检查和体积上限，再把节点交给引擎；
- 临时订阅正文每次转换结束后立即删除；
...
先看客户端里那几个会改写配置的开关是不是开着：「DNS 覆写 / DNS 设置」会顶掉整段
`dns:`（国内外分开解析、fake-ip、加密上游），「Smart 内核 / 智能分组」会改掉策略组的
类型与选路方式，「全局扩展脚本 / Merge / Script」则能改配置的任意部分。

完整配置里这几部分是配套的，规则要准就依赖它们。不了解就让这些开关保持关闭；确实
需要自己接管，再在客户端里调。

### 公开转换站转不出来

转换站要先替你把订阅拉下来，所以拉取动作是从站点的服务器发出的，不是从你的电脑。
机场那边看到的是一个陌生 IP，于是可能拒绝，或者只认特定客户端的 User-Agent。

先试自定义 User-Agent，很多机场卡的只是这个。不行再自建——而且
要跑在自己的电脑或家里的机器上，拉取才会从你自己的家宽发出。部署到 VPS 不解决这个
问题：那仍然是一个机房 IP。

### 机场后台没开启订阅

不少机场默认关着订阅功能，或者换过套餐、重置过链接之后订阅处于停用状态。这时订阅
地址本身是通的，返回的却是空内容或一句提示，转换自然出不来节点。

先去机场的用户中心确认订阅已启用、链接是当前那一条，必要时重置一次再复制。开启之后
直接刷新原来的固定地址即可，不需要重新创建档案。
...
这些信息优先来自上游 `Subscription-Userinfo`，其次来自格式完整的 `STATUS=↑/↓/TOT/Expires` 行。Shadowrocket 的完整配置引用独立节点 provider，两个响应都会透传同一份结构化用量；只有“剩余流量”节点名时仍无法可靠推算全部用量。
...
规则和站点是两样东西，改动节奏也不同，所以各走各的 tag：

| tag | 重建 | 页面上的数字 |
|---|---|---|
| `rules-v1.2.3` | 引擎镜像（rulesets、config、base） | 规则 1.2.3 |
| `site-v1.2.3` | web 镜像（网关与页面） | 站点 1.2.3 |
| `selfhost-v1.2.3` | 两个都建（拆分前的老 tag，仍可用） | 两个都变 |

这样改一行页面文案不会再重建引擎、也不会在每台跟随 `latest` 的服务器上把引擎重启一次；
反过来规则单独发版时，页面上的规则号会跟着变，而站点号不动。

**规则版本由引擎自己回答。** 它构建时在镜像里写下 `public/rules-version.json`，
网关在内网向它请求这个文件并缓存五分钟。页面因此报的是**正在跑的那份规则**，而不是
web 镜像构建时身边碰巧是哪一版；引擎没起来时页面就不显示规则号，而不是显示一个它站不住
的数字。该文件所在目录是引擎唯一对外可读的目录，`pref.yml`、规则 ini 都仍然是 404，
而且这个端口从不发布到宿主机。
